...
declare @num int
	begin
		if(select max(CONVERT(int,substring(cod_cliente,4,10))) from cliente) is not null
			...
			insert into cliente(cod_cliente,nombres,apellidos,dui,sexo,direccion,telefono,estado)
			values(@cod_cliente,@nombres,@apellidos,@dui,@sexo,@direccion,@telefeono,'1')
	end
go
...
as
update cliente set nombres=@nombres,apellidos=@apellidos,dui=@dui,sexo=@sexo,direccion=@direccion,telefono=@telefeono
...
@total decimal(18,2))
as
insert into venta(num_documento,serie,fecha,cod_cliente,dui,total,estado)
			values(@num_documento,'001',@fecha,@cod_cliente,@dui_cliente,@total,'1')
go
...
go
insert into usuario values('USER000001','Programador','12345','Administrador','1')
go
